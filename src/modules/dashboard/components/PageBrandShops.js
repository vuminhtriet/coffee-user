...
export default class PageBrandShops extends Component {
  constructor(props) {
    super(props)
  ...
  componentWillMount() {
    ...
    this.setState({ refreshing: true }, async () => {
      await getBrandShops(brandId,0, 'avgRating DESC')
      this.setState({ refreshing: false, page: 1, isLastedPage: false })
    })
  }
  ...
  onSort = (id) => {
    ...
    if (sortType.id === id) {
      ...
      this.setState({ refreshing: true, showSort: !showSort }, async () => {
        await getBrandShops(brandId, 0, newSortType.value, options)
