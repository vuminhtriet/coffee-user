...
export default class PageNearbyShops extends Component {
  constructor(props) {
    super(props)
  ...
  componentWillMount() {
    ...
    this.setState({ refreshing: true }, async () => {
      await getNearbyShops(0, 'avgRating DESC', {}, latlng)
      this.setState({ refreshing: false, page: 1, isLastedPage: false })
    })
  }
  ...
  onSort = (id) => {
    ...
    if (sortType.id === id) {
      ...
      this.setState({ refreshing: true, showSort: !showSort }, async () => {
        await getNearbyShops(0, newSortType.value, options, latlng)
