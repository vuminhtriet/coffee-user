...
export default class WriteReview extends Component {
  constructor(props) {
    ...
    const shopRating = ratings.find(item => item.memberId === userId)

    this.state = {
      shopRating,
      loading: false,
      rating: shopRating ? shopRating.rating : 0,
      title: shopRating ? shopRating.title : '',
      comment: shopRating ? shopRating.content : '',
      errors: {}
    }
  }

  submit = async () => {
    const { onBack, writeReview, editReview, token, shopId, userId, shop, getStoreInformation } = this.props
    const { rating, title, comment, shopRating } = this.state
    const errors = {}
    if (rating === 0) {
    ...
    if (!isEmpty(errors)) {
      ...
      if (!shopRating) {
        this.setState({ loading: true }, async () => {
          ...
          // await getStoreInformation(shopId)
        ...
        this.setState({ loading: true }, async () => {
          await editReview(data, token, shopRating.id)
          // await getStoreInformation(shopId)
          this.setState({ loading: false })
          onBack()
        })
