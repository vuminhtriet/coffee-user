...
export default class PaymentMethod extends Component {
  ...
  async addPayment () {
    const { token, user, addUserPayment, togglePayment, userPayments, paymentTypes } = this.props
    ...
    await addUserPayment(token, user, {
      paymentTypeId: type,
      ...values
    })
    togglePayment()
    ...
    return (
      <View style={{
        ...
        <Dropdown
          ...
          data={paymentTypes
            .filter(item => item.renderType !== PAYMENT_TYPE.COD).map(item => {
              return {
                value: item.id,
